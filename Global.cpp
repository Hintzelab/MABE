...
shared_ptr<ParameterLink<int>> Global::randomSeedPL =
    Parameters::register_parameter("GLOBAL-randomSeed", 101,
                                   ...
                                   "randomly");
shared_ptr<ParameterLink<int>> Global::updatesPL =
...
shared_ptr<ParameterLink<string>> Global::modePL =
    Parameters::register_parameter(
        "GLOBAL-mode", (string) "run",
        "mode to run MABE in [run,visualize,analyze]");

shared_ptr<ParameterLink<int>> Global::maxLineLengthPL =
...
shared_ptr<ParameterLink<string>> Global::visualizePopulationFilePL =
    Parameters::register_parameter("VISUALIZATION_MODE-visualizePopulationFile",
                                   (string) "genome_1000.csv",
                                   "which genome file do you wish to load");
shared_ptr<ParameterLink<string>> Global::visualizeOrgIDPL =
    Parameters::register_parameter(
        "VISUALIZATION_MODE-visualizeOrgIDs", (string) "[-1]",
        "ID of Genome you would like to visualize. -1 last genome file, -2 all "
        "genomes in file (world must support group evaluate)");

shared_ptr<ParameterLink<string>> Global::outputDirectoryPL =
    Parameters::register_parameter("GLOBAL-outputDirectory", (string) "./",
