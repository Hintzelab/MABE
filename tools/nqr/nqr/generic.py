...
def _prepare_condition_str(group,group_str,rep_i,iteration,groups_zcount,conditions_zcount):
    global user_module
    # rep
    rep_str = _reps_list_str[rep_i] 
    name = f"{group_str}_C{str(rep_i).zfill(conditions_zcount)}"
        ...
        for rep_i,rep in enumerate(_reps_list_str):
            for group_name,group in _groups.items():
                ...
                for i,iteration in enumerate(iterations):
                    condition_str = _prepare_condition_str(group,run['group'],rep_i,iteration,groups_zcount,conditions_zcount)
