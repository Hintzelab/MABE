...
std::shared_ptr<ParameterLink<bool>> Gate_Builder::usingCGPGatePL = Parameters::register_parameter("BRAIN_MARKOV_GATES_CARTESIANGENETICPROGRAMING-allow", false, "set to true to enable GP (what?) gates");
std::shared_ptr<ParameterLink<int>> Gate_Builder::cGPGateInitialCountPL = Parameters::register_parameter("BRAIN_MARKOV_GATES_CARTESIANGENETICPROGRAMING-initialCount", 3, "seed genome with this many start codons");
...
void Gate_Builder::setupGates() {
	...
	if (usingCGPGatePL->get(PT)) {
		inUseGateNames.insert("CartesianGeneticPrograming");
		int codonOne = CGPCode;
		inUseGateTypes.insert(codonOne);
		{
			gateStartCodes[codonOne].push_back(codonOne);
			gateStartCodes[codonOne].push_back(((1 << bitsPerCodon) - 1) - codonOne);
		}
		intialGateCounts[codonOne] = cGPGateInitialCountPL->get(PT);
		AddGate(codonOne, [](std::shared_ptr<AbstractGenome::Handler> genomeHandler, int gateID, std::shared_ptr<ParametersTable> _PT) {
			int maxIn, maxOut;
			std::pair<std::vector<int>, std::vector<int>> addresses = getInputsAndOutputs(std::string("2-2,1-1"), maxIn, maxOut, genomeHandler, gateID, _PT, "BRAIN_MARKOV_GATES_GENETICPROGRAMING");
			int operation = genomeHandler->readInt(0, 1000, AbstractGate::DATA_CODE, gateID);
			double constValue;
			for (int i = 0; i < 4; i++) {
				double constValueMin = CGPGate::constValueMinPL->get(_PT);
				double constValueMax = CGPGate::constValueMaxPL->get(_PT);
				constValue = genomeHandler->readDouble(constValueMin, constValueMax, AbstractGate::DATA_CODE, gateID);
			}
			if (genomeHandler->atEOC()) {
				std::shared_ptr<CGPGate> nullObj = nullptr;;
				return nullObj;
			}
			return std::make_shared<CGPGate>(addresses, operation, constValue, gateID, _PT);
			});
	}
