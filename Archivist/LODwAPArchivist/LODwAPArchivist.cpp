...
void LODwAPArchivist::writeLODDataFile(
	...
	//std::cout << "-------------------" << std::endl;
	//std::cout << "effective_MRCA->ID: " << effective_MRCA->ID << "  effective_MRCA->timeOfBirth: " << effective_MRCA->timeOfBirth << std::endl;
	//std::cout << "-------------------" << std::endl;

	while (next_data_write_ <= std::min( effective_MRCA->timeOfBirth, Global::updatesPL->get())) {
		// if there is convergence before thenext data interval

		// find current - this needs to be the first org with timeOfBirth < next_data_write_
		...
		time_to_coalescence = std::max(0, current->timeOfBirth - real_MRCA->timeOfBirth);
		...
		current->dataMap.writeToFile( data_file_name_, files_[data_file_name_]); // append new data to the file
...
void LODwAPArchivist::writeLODOrganismFile(
	...
	while (next_organism_write_ <= std::min(effective_MRCA->timeOfBirth, Global::updatesPL->get())) {
		// if there is convergence before the next data interval
