...
with :: so must have more then 2 characters
                        int i = 0;
                        while ((localNameSpace[i] != ':' || localNameSpace[i +
1] != ':') && (i < (int) localNameSpace.size() - 2)) {
                                i++;
                        }
...
cutting of part there is nothing left
                                        done = true;
                                }
                        }
...
characters... ie, must be at least "*::"
        ...
        if (!nameSpaceValid) {
                cout << "  Error::in nameSpaceToNameParts(const string&
...
inline vector<string> parseCSVLine(string rawLine, const char separator = ',') {
        ...
        for (auto c : rawLine) {
                if (inQuote) {  // if inQuote
                        if (c == '\"') {  // found the closed quote, we are at
the end of the quote
                                inQuote = false;
                        } else {
                                dataEntry.push_back(c);  // else just append c
to the back of dataEntry
                        }
                } else {  // if not in quote
                        if (!isspace(c)) {
                                if (c == '\"') {  // we have just entered some
quoted text, just read everything until the next quote
                                        inQuote = true;
                                } else {  // we are not inQuote and this c is
not whitespace
                                        if (c == separator) {  // if we are at a
separator
                                                dataLine.push_back(dataEntry);
// add dataEntry to dataLine
                                                dataEntry.clear();  // ... and
clear out dataEntry to make room for the next value
                                        } else {
                                                dataEntry.push_back(c);  // look
it's just text, add it to dataEntry
                                        }
                                }

                                // read non " non whitespace stuff (i.e. look
for separators and entries);

                        } else {  // it is whitespace. there should be no
whitespace in this file. Error.
                                cout << "In parseCSVLine(string rawLine)\nWhile
attempting to read csv file, encountered whitespace. This file should have no
whitespace. Exiting.\n";
                                exit(1);
                        }
                }
        }
        dataLine.push_back(dataEntry);  // we are at the end of the line. add
dataEntry to dataLine
        dataEntry.clear();  // ... and clear out dataEntry to make room for the
next value
        return dataLine;
}
*/
...
inline map<string, vector<string>> readFromCSVFile(const string& fileName, const
char separator = ',') {
        std::ifstream FILE(fileName);
        map<string, vector<string>> data;  // the final map
        vector<string> dataLine;  // one line of data, converted from a single
csv string to a vector of strings
        string rawLine;
        bool firstLine = true;
        vector<string> lookUpTable;

        if (FILE.is_open())  // if the file named by configFileName can be
opened
        {
                dataLine.clear();
                while (getline(FILE, rawLine))  // keep loading one line from
the file at a time into "line" until we get to the end of the file
                {
                        dataLine = parseCSVLine(rawLine, separator);
                        if (firstLine) {  // this is the first line, dataLine
contains the keys... use them to build the map and loopUpTable
                                for (auto key : dataLine) {
                                        if (isspace(key.back())){
                                                key.pop_back();
                                        }
                                        lookUpTable.push_back(key);  // add the
key so we can make sure we assign the right values to the right columns
                                        data[key] = {};  // add an empty vector
for each key into the map data
                                }
                                firstLine = false;
                        } else {  // we are not in the first line, dataLine has
values
                                for (size_t i = 0; i < dataLine.size(); i++) {
// for each entry in dataLine
...
index
inline string CSVLookUp(map<string, vector<string>> CSV_Table, const string&
lookupKey, const string& lookupValue, const string& returnKey) {
        ...
        if (iter == CSV_Table.end()) {
                throw std::invalid_argument("CSVLookup could not find requested
lookup key\n");
        }
...
return key\n");
        ...
        // if the lookup and return keys exist, look for the look up value in
the lookup keys vector
...
template<class T>
void convertCSVListToVector(string stringData, vector<T> &returnData, const char
separator = ',') {
        returnData.clear();
        if (stringData[0] == '\"') {
...
strip off leading and trailing quotes
        ...
        if (stringData[0] == '[') {
                stringData = stringData.substr(1, stringData.size() - 2);  //
strip off leading and trailing square brackets
        }
        if (stringData.size() != 0) {  // this is not an empty list... if it is
an empty list, we don't need to do anything else
                ...
                for (auto s : dataLine) {
                        ...
                        if (!success) {
                                cout << " --- while parsing: " << stringData <<
" .... " << endl;
                                cout << " In convertCSVListToVector() attempt to
convert string to value failed\n" << endl;
                                exit(1);
                        } else {
...
inline void convertCSVListToVector(std::string string_data,
  ...
  if (!std::regex_match(string_data, m, stripoff)) {
    std::cout << " Error :  unrecognized csv string " << string_data
              << "in convertCSVListToVector " << std::endl;
    exit(1);
  }
  string_data = m[1].str();
...
/*
template<typename Type>
inline int vectorToBitToInt(const vector<Type> &nodes, const vector<int>
&nodeAddresses, bool reverseOrder = false) {
        int result = 0;
        if (reverseOrder) {
...
template<typename Type>
inline int vectorToTritToInt(const vector<Type> &nodes, const vector<int>
&nodeAddresses, bool reverseOrder = false) {
        int result = 0;
        if (reverseOrder) {
                for (int i = (int) nodeAddresses.size() - 1; i >= 0; i--) {
                        result = (result * 3) + (Trit(nodes[nodeAddresses[i]]) +
1);
                }
        } else {
                for (int i = 0; i < (int) nodeAddresses.size(); i++) {
...
/*
inline vector<int> seq(const string seqStr, int defaultMax = -1, bool addZero =
false) {
        stringstream ss(seqStr);
        int n;
        char c;
        bool error = (seqStr.size() == 0) ? true : false;  // detect empty
string
        ...
        while (!ss.fail() && !error) {  // while not at end of string and no
error has been detected
                ...
                if (ss.fail()) {  // if first element read was not an int
                        ss.clear();
                        ss >> c;
                        if (c == ':') {  // this may be a definition of format
':x', that is a range from 0
                                ss >> n;
                                if (ss.fail()) {  // we were expecting an int!
                                        error = true;
                                } else {  // set up seq definition 0 >
defaultMax : n
                                        if (defaultMax == -1) {
                                                cout << "Error in seq(const
string seqStr, int defaultMax = -1)\n  attempt to use simple range (':x'), but
defaultMax was not set\n  will return empty sequence."<<endl;
                                                error = true;
                                        }
                        ...
                        seqDefinitions[currentDef].push_back(n);  // push back
first number read
                        ss >> c;
                        if (c == ',' || ss.fail()) {
                                // if next char is a ',' or end of string, we
have a single value or a ':x' definition
                                // do nothing...
                        } else if (c == '-') {
                                ss >> n;
                                seqDefinitions[currentDef].push_back(n);
                                ...
                                if (c == ':') {
                                        ...
                                        //we are at the end of this definition
(either comma, get read to read more, or end of string)
                                        seqDefinitions[currentDef].push_back(1);
// no step was provided
                                } else {
                                        error = true;  // we did not find a
comma or end of string at the end of a definition
                                }
                        } else if (c == ':') {  // this maybe a definition of
format x : y
                                ss >> n;
                                if (ss.fail()) {  // we were expecting an int!
                                        error = true;
...
defaultMax : n
                                        if (defaultMax == -1) {
                                                cout << "ERROR :: in seq(const
string seqStr, int defaultMax = -1)\n  attempt to define unbound range ('x:y'),
but defaultMax was not set\n  will return empty sequence."<<endl;
                                                error = true;
                                        }
                                        seqDefinitions[currentDef].push_back(defaultMax);
// first is alreay pushed
                                        seqDefinitions[currentDef].push_back(n);
                                        ss >> c;
                                        if (c != ',' && !ss.fail()) {
                                                error = true;
                                ...
                                error = true;  // we needed to find either '>'
or ',' and did not!
                        }
                }
                currentDef++;
        ...
        if (addZero){
                seq.push_back(0); // may result in duplicate which will be
...
inline void loadIndexedCSVFile(const string& fileName,
unordered_map<int,unordered_map<string,string>>& data, string& indexName) {
        data.clear();
        std::ifstream FILE(fileName);
        string rawLine;
...
opened
                ...
                numKeys = keys.size();
                // find indexIndex
                auto index = find(keys.begin(), keys.end(), indexName) -
keys.begin();
...
--  " << flush;
                ...
                cout << "\n\nERROR: In MultiGenome::loadGenomeFile, unable to
open file \"" << fileName << "\"\n\nExiting\n" << endl;
                exit(1);
        }
}
...
inline string GetStringOfVector(const string &key) { // retrieve a string from a
dataMap with "key" - if not already string, will be converted
                string returnString = "\"[";
                dataMapType typeOfKey = findKeyInData(key);
                if (typeOfKey == NONE) {
                        cout << "  In DataMap::GetString() :: key \"" << key <<
"\" is not in data map!\n  exiting." << endl;
                        exit(1);
                } else {
                        ...
                        } else if (typeOfKey == DOUBLE || typeOfKey ==
DOUBLESOLO) {
                        ...
                        } else if (typeOfKey == STRING || typeOfKey ==
STRINGSOLO) {
