...
class ANNbackpropBrain: public AbstractBrain {
public:
	...
	ANNbackpropBrain() = delete;

	ANNbackpropBrain(int _nrInNodes, int _nrOutNodes, std::shared_ptr<ParametersTable> _PT = Parameters::root);

	virtual ~ANNbackpropBrain() = default;
...
inline std::shared_ptr<AbstractBrain> ANNbackpropBrain_brainFactory(int ins, int outs, std::shared_ptr<ParametersTable> PT) {
	return std::make_shared<ANNbackpropBrain>(ins, outs, PT);
}
