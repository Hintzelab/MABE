...
ClassicGateListBuilder::buildGateListAndGetAllValues(
    ...
    std::vector<std::vector<int>> &genomePerGateValues, int genomePerGateValuesCount,
    std::shared_ptr<ParametersTable> gatePT) {
  ...
  if (genome->isEmpty()) {
    ...
    while (!translation_Complete) {
      if (genomeHandler->atEOC()) { // if genomeIndex > testIndex, testIndex has
        ...
        gateGenomeHandler->readInt(0,
                                   codonMax); // move back 2 start codon values
        ...
        gateGenomeHandler->readInt(0, codonMax, AbstractGate::START_CODE,
                                   gateCount);
        std::shared_ptr<AbstractGate> newGate = gateBuilder.makeGate[testSite1Value](
            gateGenomeHandler, gateCount, gatePT);
