...
std::shared_ptr<ParameterLink<std::string>> ProbabilisticGate::IO_RangesPL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_PROBABILISTIC-IO_Ranges", (std::string) "1-4,1-4",
        "range of number of inputs and outputs (min inputs-max inputs,min "
        "outputs-max outputs)");

ProbabilisticGate::ProbabilisticGate(std::pair<std::vector<int>, std::vector<int>> addresses,
...
void ProbabilisticGate::update(
    std::vector<double> &nodes,
    std::vector<double> &nextNodes) { // this translates the input bits of the
                                 // current states to the output bits of the
                                 // next states
  ...
  double r = Random::getDouble(
      1); // r will determine with set of outputs will be chosen
  while (r > table[input][outputColumn]) {
    r -= table[input][outputColumn]; // this goes across the probability table
                                     // in row for the given input and subtracts
                                     // each
    // value in the table from r until r is less than a value it reaches
    outputColumn++; // we have not found the correct output so move to the next
                    // output
  }
  for (size_t i = 0; i < outputs.size(); i++) // for each output...
    nextNodes[outputs[i]] += 1.0 * ((outputColumn >> (outputs.size() - 1 - i)) &
                                    1); // convert output (the column number) to
                                        // bits and pack into next states
  // but always put the last bit in the first input (to maintain consistancy)
}

std::shared_ptr<AbstractGate>
ProbabilisticGate::makeCopy(std::shared_ptr<ParametersTable> PT_) {
  if (PT_ == nullptr) {
    PT_ = PT;
  }
  auto newGate = std::make_shared<ProbabilisticGate>(PT_);
  newGate->table = table;
  newGate->ID = ID;
