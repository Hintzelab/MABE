...
std::shared_ptr<ParameterLink<std::string>> DecomposableGate::IO_RangesPL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_DECOMPOSABLE-IO_Ranges", (std::string) "1-4,1-4",
...
void DecomposableGate::update(
    std::vector<double> &nodes,
    std::vector<double> &nextNodes) {                // this translates
                                                     // the input bits of
                                                     // the current states
                                                     // to the output bits
                                                     // of the next states
