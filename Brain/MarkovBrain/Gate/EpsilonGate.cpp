 ...
 * error which may the gate to deliver the wrong set of output values */

std::shared_ptr<ParameterLink<double>> EpsilonGate::EpsilonSourcePL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_EPSILON-epsilonSource", 0.05,
        "if value is in ance tha[0,1], cht output will be randomized. if < 1 "
...
std::shared_ptr<ParameterLink<std::string>> EpsilonGate::IO_RangesPL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_EPSILON-IO_Ranges", (std::string) "1-4,1-4",
        "range of number of inputs and outputs (min inputs-max inputs,min "
        "outputs-max outputs)");

EpsilonGate::EpsilonGate(std::pair<std::vector<int>, std::vector<int>> addresses,
  ...
  // now to the specifics of this gate - we convert the table to a list of
  // numbers (i.e. bitstrings) so we can do fast comparisons in the update
  defaultOutput
      .clear(); // clear the defaultOutput vector... the list of bitstrings
  for (size_t i = 0; i < table.size();
       i++) { // for every row of the table (which is 2^inputs)
    int D = 0;
    for (size_t j = 0; j < table[i].size(); j++) { // for every space in the row
      D |= (Bit(table[i][j]) << j); // translate the value in that space to a
                                    // bit (0 or 1), then bitshift it [position
                                    // in row] times, then take D, bit-or it
                                    // with that number,
      // and make that the new D
    }
    defaultOutput.push_back(D); // add D (which is a bitstring of length
                                // [2^outputs]) to defaultOutput. defaultoutputs
                                // is now of length (2^number of inputs)
  }
}
...
void EpsilonGate::update(std::vector<double> &nodes, std::vector<double> &nextNodes) {
  int input = vectorToBitToInt(nodes, inputs, true); // converts the input
                                                     ...
                                                     // (true indicates to
                                                     // reverse order)
  if (Random::P(epsilon)) { // if chance... then return a different set of
                            // output values
    int output = 0;
    do {
      output = Random::getIndex(table.size());
    } while (output == defaultOutput[input]); // keep picking randomly until you
...
EpsilonGate::makeCopy(std::shared_ptr<ParametersTable> PT_) {
  if (PT_ == nullptr) {
    PT_ = PT;
  }
  auto newGate = std::make_shared<EpsilonGate>(PT_);
  newGate->table = table;
  newGate->ID = ID;
