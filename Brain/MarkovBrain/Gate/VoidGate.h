...
public:
  ...
  double epsilon;
  VoidGate() = delete;
  VoidGate(std::shared_ptr<ParametersTable> PT_ = nullptr) : DeterministicGate(PT_) {
    epsilon = 0;
  }
  VoidGate(std::pair<std::vector<int>, std::vector<int>> addresses,
  ...
  virtual std::string gateType() override { return "Void"; }
