...
std::shared_ptr<ParameterLink<std::string>>
    DecomposableFeedbackGate::IO_RangesPL = Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_DECOMPOSABLE_FEEDBACK-IO_Ranges",
        (std::string) "1-4,1-4", "range of number of inputs and outputs (min "
                                 "inputs-max inputs,min outputs-max outputs)");

DecomposableFeedbackGate::DecomposableFeedbackGate(
    ...
    : AbstractGate(PT_), factors(factors), ins(addresses.first.size()),
      outs(addresses.second.size()) {
  ...
  for (i = 0; i < (1 << numInputs);
       i++) { // for each row (each possible input bit std::string)
...
void DecomposableFeedbackGate::update(std::vector<double> &states,
  ...
  if ((feedbackON) && (nrPos != 0) && (states[posFBNode] > 0.0)) {
    for (i = 0; i < chosenInPos.size(); i++) {
      ...
      if (((chosenOutPos[i] >> randomFactori) & 1) == 1) {
        ...
        factors[chosenInPos[i]][randomFactori] -= mod;
        factors[chosenInPos[i]][randomFactori] =
            std::max(factors[chosenInPos[i]][randomFactori], 0.);
      }
      ...
      for (int outputi = 0; outputi < table[rowi].size(); outputi++) {
        ...
        /// the appropriate factors together in the right way (1-p) for bit=0 vs
        /// p for bit=1
      ...
      double rowSum(accumulate(begin(table[chosenInPos[i]]),
                               end(table[chosenInPos[i]]), 0.)); /// sum row
      for (auto &number : table[chosenInPos[i]])
        number /= rowSum; /// divide row by sum
  ...
  if ((feedbackON) && (nrNeg != 0) && (states[negFBNode] > 0.0)) {
    for (i = 0; i < chosenInNeg.size(); i++) {
      ...
      if (((chosenOutNeg[i] >> randomFactori) & 1) == 1) {
        ...
        factors[chosenInNeg[i]][randomFactori] =
            std::max(factors[chosenInNeg[i]][randomFactori], 0.);
      } else {
        factors[chosenInNeg[i]][randomFactori] += mod;
      }
      ...
      double rowSum(accumulate(begin(table[chosenInNeg[i]]),
                               end(table[chosenInNeg[i]]), 0.)); /// sum row
      for (auto &number : table[chosenInNeg[i]])
        number /= rowSum; /// divide row by sum
      // transform( begin(table[chosenInNeg[i]]), end(table[chosenInNeg[i]]),
      // begin(table[chosenInNeg[i]]), bind1st(divides<T>(), rowSum) ); ///
      // divide row by sum
...
void DecomposableFeedbackGate::applyNodeMap(std::vector<int> nodeMap,
                                            int maxNodes) {
...
//    std::vector<std::vector<double>> normalizedTable;
...
DecomposableFeedbackGate::makeCopy(std::shared_ptr<ParametersTable> PT_) {
  if (PT_ == nullptr) {
    PT_ = PT;
  }
  auto newGate = std::make_shared<DecomposableFeedbackGate>(PT_);
