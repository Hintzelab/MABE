...
public:
  ...
  FeedbackGate() = delete;
  FeedbackGate(std::shared_ptr<ParametersTable> PT_ = nullptr)
      : AbstractGate(PT_) {
    table = {};
  }
  virtual ~FeedbackGate() = default;
  virtual std::string gateType() override { return "Feedback"; }
  ...
  FeedbackGate(std::pair<std::vector<int>, std::vector<int>> addresses,
               std::vector<std::vector<int>> rawTable, unsigned int _posFBNode,
               unsigned int _negFBNode, unsigned char _nrPos,
               unsigned char _nrNeg, std::vector<double> _posLevelOfFB,
               std::vector<double> _negLevelOfFB, int ,
