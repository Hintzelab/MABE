...
std::shared_ptr<ParameterLink<double>> GPGate::constValueMinPL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_GENETICPROGRAMING-constValueMin", -1.0,
        "for the const values, min value the genome can generate");

std::shared_ptr<ParameterLink<double>> GPGate::constValueMaxPL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_GENETICPROGRAMING-constValueMax", 1.0,
        "for the const values, max value the genome can generate");
std::shared_ptr<ParameterLink<std::string>> GPGate::IO_RangesPL =
    Parameters::register_parameter(
        "BRAIN_MARKOV_GATES_GENETICPROGRAMING-IO_Ranges", (std::string) "1-4,1-4",
        "range of number of inputs and outputs (min inputs-max inputs,min "
        "outputs-max outputs)");

/* *** GP Gate implementation *** */

// GPGate::GPGate(shared_ptr<AbstractGenome> genome,
// shared_ptr<AbstractGenome::Handler> genomeHandler, int gateID) {
GPGate::GPGate(std::pair<std::vector<int>, std::vector<int>> _addresses, int _operation,
...
std::string GPGate::description() {
  ...
  std::string gateTypeName[9] = {
      "fixed constants", "+", "-", "*", "/", "Sin", "Cos", "Log", "Exp"};
  std::string constString = " constants: " + std::to_string(constValues[0]) + " " +
                       std::to_string(constValues[1]) + " " +
                       std::to_string(constValues[2]) + " " +
                       std::to_string(constValues[3]) + "\n";
  return "Gate " + std::to_string(ID) + " is a (" + gateTypeName[operation] + ") " +
         gateType() + "Gate\n" + AbstractGate::descriptionIO() + constString;
}

std::shared_ptr<AbstractGate> GPGate::makeCopy(std::shared_ptr<ParametersTable> PT_) {
  if (PT_ == nullptr) {
    PT_ = PT;
  }
  auto newGate = std::make_shared<GPGate>(PT_);
  newGate->ID = ID;
